...
def sample_img(img: np.ndarray, sample_loc: dict, size: int = 3, save_crop: bool = False) -> dict:
    ...
    for loc_, loc_coord in sample_loc.items():
        ...
        img_crop_hsv = color.rgb2hsv(img_crop).astype('float16')
...
save_path_moth_hsv_ = f'../../data/imgs_hsv_samples_grid.csv'
...
for id, index in index_id.items():
    imgs = []
    size = 400 if len(index)>400 else len(index)
    index_ = np.random.choice(index, size=size, replace=False)
    
    print(f'\tClustering id : {id}, size : {len(index)}, random choice {len(index_)} examples')
    ...
    plot_imgs_cluster(imgs=imgs_numpy, id=id, index=index_, df_label=df_label)
