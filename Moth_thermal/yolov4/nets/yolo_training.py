...
def box_ciou(b1, b2):
    ...
    # 對於預測框小於真實框給予額外的懲罰
    # b1->pred, b2->true
    b_mins_diff_ = (b1_mins - b2_mins) / b2_wh
    b_mins_diff = b_mins_diff_.clone()
    b_mins_diff[b_mins_diff_ < 0] = 0
    b_mins_penalty = torch.sum(torch.sqrt(b_mins_diff), -1)

    b_maxes_diff_ = (b2_maxes - b1_maxes) / b2_wh
    b_maxes_diff = b_maxes_diff_.clone()
    b_maxes_diff[b_maxes_diff_ < 0] = 0
    b_maxes_penalty = torch.sum(torch.sqrt(b_maxes_diff), -1)
    ...
    # Mai edit @ 20210318
    # ciou = ciou - alpha * v
    ciou = diou - alpha * v - (b_mins_penalty + b_maxes_penalty) / 2
    return ciou
