...
class YOLO(object):
    _defaults = {
        ...
        "model_path": 'logs/Epoch150-Total_Loss0.4539-Val_Loss0.3995.pth',
        "anchors_path": 'model_data/yolo_anchors_640.txt',
    ...
    def detect_image(self, image):
        ...
        for i, c in enumerate(top_label):
            ...
            left = max(0, left)
            top = max(0, top)
            right = min(image_original.width - 1, right)
            bottom = min(image_original.height - 1, bottom)

            bboxes.append(
                ','.join([str(left), str(top), str(right), str(bottom)]))
            bbox_cropped_images.append(
                image_original.crop((left, top, right, bottom)))
            ...
            del draw
        return image, bbox_cropped_images, ' '.join(bboxes)
    ...
    def predict_bboxes(self, image):
        ...
        for i, c in enumerate(top_label):
            top, left, bottom, right = boxes[i]
            bboxes.append(','.join([left, top, right, bottom]))
