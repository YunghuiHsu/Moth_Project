...
def restrict_boundry(bboxes, img):
    for i in [0, 1]:
...
def get_bgcolor(img, pixel_size=20):
    ...
    h, w = img_.shape[:2]
    center_top = img_[0:20, int(w*0.5 - 2*pixel_size/2)
                                : int(w*0.5 + 2*pixel_size/2)]
    bottom_left = img_[-5-pixel_size:-5, : int(2*pixel_size)]
    bottom_right = img_[-5-pixel_size:-5, -int(2*pixel_size):]
