...
import cv2
from aug_rotation import YoloRotatebbox
...
class YoloDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        h, w = input_shape
        image_path, bboxes_str = annotation_line.split('\t')
        # 保存框的位置
        if self.rand() < .5:
            rotate_angle = np.random.randint(-5, 5)
            yolo_rotate_bbox = YoloRotatebbox(annotation_line, rotate_angle)
            box = np.array(yolo_rotate_bbox.rotateYolobbox())
            image = Image.fromarray(yolo_rotate_bbox.rotate_image())
        else:
            try:
                image = Image.open(image_path)
            except:
                raise Exception('Failed on loading: ', image_path)
    ...
    def get_random_data_with_Mosaic(self, annotation_lines, input_shape, hue=.1, sat=1.5, val=1.5):
        h, w = input_shape
        min_offset_x = 0.3
        min_offset_y = 0.3
