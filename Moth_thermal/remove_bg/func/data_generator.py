...
def open_y_images(*paths, im_h=None, im_w=None, gray=False):
    ...
    try:
        ...
        if (im_h is not None and isinstance(im_h, int) and
            ...
            #                 k = np.array(k)
            #                 print(k.shape)
            data = np.array([np.array(Image.open(i).convert(
                'L').resize((w, h)), dtype='float32') for i in paths])
        else:
            data = np.array([np.array(Image.open(i), dtype='float32')
                            for i in paths])
...
def open_images(*paths, im_h=None, im_w=None, gray=False):
    ...
    try:
        ...
        if (im_h is not None and isinstance(im_h, int) and
                im_w is not None and isinstance(im_w, int)):
            data = np.array(
                [np.array(Image.open(i).resize((w, h)), dtype='float32') for i in paths])
        else:
            data = np.array([np.array(Image.open(i), dtype='float32')
                            for i in paths])
    except Exception as e:
...
def get_label_path(imgdir_path, labeldir_path, task_type):
    ...
    # multilabel.npy #mothmask.npy
    return os.path.join(labeldir_path, basename, task_type+'.npy')
