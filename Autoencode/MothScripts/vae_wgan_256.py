...
'''
將尺寸從64*64改為 256*256
注意，修改的是feature map輸入與輸出的尺寸而非數量
'''
...
class UnFlatten(nn.Module): 
    def forward(self, input: Tensor, channel: int = 512, H: int = 4*4, W: int = 4*4):  # 將寬與高各*4，使進入decoder的特徵圖尺寸可以變為16倍
...
class VAE_WGAN(nn.Module):
    def __init__(self, 
        ...
        self.fc1 = nn.Linear(512*4*4*16, z_dim)  # *16倍，for 64x64 > 256x256
        self.fc2 = nn.Linear(512*4*4*16, z_dim)  # *16倍，for 64x64 > 256x256
        self.fc3 = nn.Linear(z_dim, 16*512*4*4)  # *16倍，for 64x64 > 256x256
        ...
        self.discriminator = nn.Sequential(
            ...
            nn.Conv2d(512, 1, kernel_size=4*4, stride=4*4, bias=False),  # kernel 跟stride都*4, 讓判別器的輸出縮小4*4倍
            nn.BatchNorm2d(1),
...
#         std = logvar.mul(0.5).exp_()
        ...
        fo1, *_ = vgg(x)
        fr1, *_ = vgg(recon_x)
        y = self.discriminator(torch.cat((fo1, fr1), 0))  # 輸入vgg特徵萃取的第一層
